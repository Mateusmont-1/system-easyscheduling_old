...
from . import tela_cadastro_folga
from . import tela_verificar_folgas
...
class UserWidget(flet.UserControl):
    ...
    def build(self):
        ...
        self._register_day_off = self.Create_button("Cadastrar folga", self.func, self._visible_create_collaborator)
        self._check_day_off = self.Create_button("Verificar folga", self.func2, self._visible_edit_collaborator)
        self._back = self.Create_button("Voltar", self.func3, self._visible_back_)
        ...
        return flet.Column(
            ...
            controls=[
                ...
                flet.Column(
                    ...
                    flet.Row(
                        alignment="center",
                        controls=[self._register_day_off]),
                    flet.Row(
                        alignment="center",
                        controls=[self._check_day_off]),
                    flet.Row(
                        alignment="center",
                        controls=[self._back]),
                    ]),
...
async def main(page:flet.page, user):
    ...
    async def _tela_register_day_off(e):
        await tela_cadastro_folga.main(page, user)
    
    async def _tela_check_day_off(e):
        await tela_verificar_folgas.main(page, user)
