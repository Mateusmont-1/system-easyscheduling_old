...
from . import tela_cadastro_categoria
from . import tela_verificar_categoria
from . import tela_verificar_despesa
...
class UserWidget(flet.UserControl):
    def __init__(
        ...
        self._visible_create_category = False
        self._visible_check_category = False
        self._visible_check_expense = False
        ...
        if self.user['funcaoID'] == "colaborador":
            ...
            self._visible_create_category = True
            self._visible_check_category = True
            self._visible_check_expense = True
    ...
    def build(self):
        ...
        self._create_category = self.Create_button("Adicionar categoria", self.func, self._visible_create_category)
        self._check_category = self.Create_button("Verificar categorias", self.func2, self._visible_check_category)
        self._check_expense = self.Create_button("Verificar despesas", self.func3, self._visible_check_expense)
        ...
        return flet.Column(
            ...
            controls=[
                ...
                flet.Column(
                    ...
                    flet.Row(
                        alignment="center",
                        controls=[self._create_category]),
                    flet.Row(
                        alignment="center",
                        controls=[self._check_category]),
                    flet.Row(
                        alignment="center",
                        controls=[self._check_expense]),
                    flet.Row(
                        alignment="center", 
...
async def main(page:flet.page, user):
    ...
    async def _create_category(e):
        await tela_cadastro_categoria.main(page,user)
        # tela_cadastro_servico.main(page, user)
        
    async def _check_category(e):
        await tela_verificar_categoria.main(page, user)
    
    async def _check_expense(e):
        await tela_verificar_despesa.main(page, user)
