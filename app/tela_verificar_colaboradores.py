...
from . import tela_editar_colaborador
    ...
    def build(self):
        ...
        self.data_table = flet.Container(
            ...
            content=flet.Row(
                ...
                columns=[
                    flet.DataColumn(flet.Text("Nome Colaborador",
                                              text_align="center",
                                              weight="bold",
                                              color=COLOR_TEXT_IN_FIELD)),
                    flet.DataColumn(flet.Text("Permitir agendamento",
                                              text_align="center",
                                              weight="bold",
                                              color=COLOR_TEXT_IN_FIELD)),
                ],
        ...
        for colaborador_ in self.collaborator_ref:
            ...
            self.data_table.content.controls[0].rows.append(flet.DataRow(cells=[
                    flet.DataCell(flet.Text(colaborador['nome'],
                                            text_align="center",
                                            weight="bold",
                                            color=COLOR_TEXT_IN_FIELD,
                                            )),
        ...
        self.no_scheduling = flet.Container(
            ...
            alignment=flet.alignment.center,
            content=flet.Text(
                value="Não possui colaboradores",
                size=30,
                text_align="center",
                weight="bold",
                color=COLOR_TEXT_IN_FIELD,
...
async def main(page:flet.page,user):
    page.title = "Verificar colaboradores"
    # page.bgcolor = "#f0f3f6"
    ...
    def show_details(row_data, id):
        ...
        dialog.content.controls = [
            flet.Text(f"Nome Colaborador: {row_data['nome']}"),
            flet.Text(f"Permitir agendamento: {texto}"),
            flet.Text(f"Segunda a sexta: {row_data['dias_uteis']}"),
            flet.Text(f"Sábado: {row_data['sabado']}"),
            flet.Text(f"Domingo: {row_data['domingo']}"),
        ]
        dialog.open = True
        id_colaborador = id
        page.update()

    async def button_editar_colaborador(e):
        close_dialog()
        await asyncio.sleep(0.1)
        await tela_editar_colaborador.main(page,user, id_colaborador)
        
    id_colaborador = None
    # Criar o diálogo uma vez
    dialog = flet.AlertDialog(
        scrollable=True,
        title=flet.Text("Detalhes do Colaborador"),
        content=flet.Column([]),
        actions=[
            flet.TextButton("Editar colaborador", on_click=button_editar_colaborador),
    ...
    page.on_resize = page_resize_width
    
    def page_resize_heigh(e=None):
        if e:
            if page.height > 600:     
                altura = 600
    ...
    page.on_resize = page_resize_heigh
                
    def _main_column_():
        return flet.Container(
            width=page_resize_width(),
            height=page_resize_heigh(),
