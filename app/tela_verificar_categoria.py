...
from . import tela_editar_categoria
...
class UserWidget(flet.UserControl):
    def __init__(
        ...
        super().__init__()
        self.title = title
        self.categoria_ref = produto_ref
        self.func = func1
        self.func2 = func2
    ...
    def build(self):
        ...
        self.data_table = flet.Container(
            ...
            content=flet.Row(
                ...
                columns=[
                    flet.DataColumn(flet.Text("Nome categoria",
                                              text_align="center",
                                              weight="bold",
                                              color=COLOR_TEXT_IN_FIELD,)),
                    flet.DataColumn(flet.Text("Ativo",
                                              text_align="center",
                                              weight="bold",
                                              color=COLOR_TEXT_IN_FIELD)),
                ],
        ...
        for categoria_ in self.categoria_ref:
            ...
            self.data_table.content.controls[0].rows.append(flet.DataRow(cells=[
                    flet.DataCell(flet.Text(categoria['nome'],
                                            text_align="center",
                                            weight="bold",
                                            color=COLOR_TEXT_IN_FIELD,
                                            )),
...
async def main(page:flet.page, user):
    page.title = "Verificar categorias"
    # page.bgcolor = "#f0f3f6"
    ...
    categoria_ref = db.collection("categorias").stream()
    
    def show_details(row_data, id):
        nonlocal id_categoria
        ...
        dialog.content.controls = [
            flet.Text(f"Nome Categoria: {row_data['nome']}"),
            flet.Text(f"Ativo: {texto}"),
        ]
        dialog.open = True
        id_categoria = id
        page.update()
 
    async def button_editar_categoria(e):
        close_dialog()
        await asyncio.sleep(0.1)
        await tela_editar_categoria.main(page,user, id_categoria)  
        
    id_categoria = None
    # Criar o diálogo uma vez
    dialog = flet.AlertDialog(
        scrollable=True,
        title=flet.Text("Detalhes da categoria"),
        content=flet.Column([]),
        actions=[
            flet.TextButton("Editar categoria", on_click=button_editar_categoria),
