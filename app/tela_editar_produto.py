...
class UserWidget(flet.UserControl):
    def __init__(
        ...
        self.erro = erro
        self.produto_ref = produto_ref.to_dict()
        self.func = func
        self.func2 = func2
        super().__init__()
        
        self.produto_id = produto_ref.id
    ...
    def build(self):
        ...
        self._checkbox = flet.Container(
            ...
            content=flet.Row(
                controls=[
                flet.Checkbox(value=self.produto_ref['permitir_venda']),
                flet.Text(
                ...
                flet.Column(
                    ...
                    controls=[
                        self.InputTextField("Nome",
                                            False, self.produto_ref['nome']),
                        self.InputTextField("Preço (R$)",
                                            False, str(self.produto_ref['preco'])),
                    ],
                ),
                self._checkbox,
...
async def main(page:flet.page, user, id_produto):
    ...
    # id_produto = "9SOdgwhLrFJ8jrl4Gz8d"
    db = get_firestore_client()
    produto_ref = db.collection("produto").document(id_produto).get()
