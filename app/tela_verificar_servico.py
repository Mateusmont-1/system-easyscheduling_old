...
from . import tela_editar_servico
...
class UserWidget(flet.UserControl):
    ...
    def build(self):
        ...
        self.data_table = flet.Container(
            ...
            content=flet.Row(
                ...
                controls=[flet.DataTable(
                    columns=[
                        flet.DataColumn(flet.Text("Nome serviço",
                                              text_align="center",
                                              weight="bold",
                                              color=COLOR_TEXT_IN_FIELD)),
                        flet.DataColumn(flet.Text("Preço",
                                              ...
                                              weight="bold",
                                              color=COLOR_TEXT_IN_FIELD)),
                        flet.DataColumn(flet.Text("Duração",
                                              text_align="center",
                                              weight="bold",
                                              color=COLOR_TEXT_IN_FIELD)),
                ],
        ...
        for servico_ in self.produto_ref:
            servico_id = servico_.id
            servico = servico_.to_dict()

            self.data_table.content.controls[0].rows.append(flet.DataRow(cells=[
                    flet.DataCell(flet.Text(servico['nome'],
                                            text_align="center",
                                            weight="bold",
                                            color=COLOR_TEXT_IN_FIELD,
                                            )),
                    flet.DataCell(flet.Text(servico['preco'],
                                            text_align="center",
                                            weight="bold",
                                            color=COLOR_TEXT_IN_FIELD)),
                    flet.DataCell(flet.Text(servico['duracao'],
                                            text_align="center",
                                            weight="bold",
                                            color=COLOR_TEXT_IN_FIELD)),
                ],
...
async def main(page:flet.page, user):
    ...
    def show_details(row_data, id):
        ...
        dialog.content.controls = [
            flet.Text(f"Nome Serviço: {row_data['nome']}"),
            flet.Text(f"Preço: {row_data['preco']}"),
            flet.Text(f"Permitir agendamento: {texto}"),
            flet.Text(f"Duração: {row_data['duracao']} min"),
        ]
        dialog.open = True
        id_servico = id
        page.update()
    ...
    async def button_editar_servico(e):
        ...
        await tela_editar_servico.main(page,user, id_servico)  

    id_servico = None
    # Criar o diálogo uma vez
    dialog = flet.AlertDialog(
        scrollable=True,
        title=flet.Text("Detalhes do Serviço"),
        content=flet.Column([]),
        actions=[
            flet.TextButton("Editar serviço", on_click=button_editar_servico),
